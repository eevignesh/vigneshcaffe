 ...
 * and 2. mean ap for a multi way classification task.
 */
template <typename Dtype>
class ClassificationStatsLayer : public Layer<Dtype> {
 public:

  explicit ClassificationStatsLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_CLASSIFICATION_STATS;
  }
 ...
 protected:
   ...
   *      indicating the correct class label among the @f$ K @f$ classes
   * @param top output Blob vector (length 2), add more if needed
   * for now we have only accuracy and mean ap
   *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
...
/**
 * @brief Computes the max-margin loss mainly for rank loss applications.
 *
 * @param bottom input Blob vector (length 2)
 *   -# @f$ (N \times C \times H \times W) @f$
 *      the predictions @f$ y^{true} @f$, a Blob with values in
 *      @f$ [-\infty, +\infty] @f$ indicating the "true" predicted score for each of
 *      the @f$ K = NCHW @f$ samples.  
 *
 *   -# @f$ (N \times C \times H \times W) @f$
 *      the predictions @f$ y^{bogus} @f$, a Blob with values in
 *      @f$ [-\infty, +\infty] @f$ indicating the "wrong or bogus" predicted score for each of
 *      the @f$ K = NCHW @f$ samples.
 *
 * @param top output Blob vector (length 1)
 *   -# @f$ (1 \times 1 \times 1 \times 1) @f$
 *      the computed hinge loss: @f$ E =
 *        \frac{1}{K} \sum\limits_{k=1}^K
 *        [\max(0, 1 - \delta + y^{bogus}_k - y^{true}_k)] ^ p
 *      @f$, for the @f$ L^p @f$ norm
 *      (defaults to @f$ p = 1 @f$, the L1 norm; L2 norm, as in L2-SVM,
 *      is also available), and @f$
 *      \delta\{\mathrm{condition}\} = \left\{
 ...
 */
template <typename Dtype>
class MaxMarginLossLayer : public LossLayer<Dtype> {
 public:
  explicit MaxMarginLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
 ...
 protected:
   ...
   * @brief Computes the hinge loss error gradient w.r.t. the predictions.
   *
   * Gradients are computed for both inputs.
